...
def skew2(sigma_matrix, grid, alpha, theta):
    inverse_sigma = np.linalg.inv(sigma_matrix)
    vect = np.sum(np.dot(grid, inverse_sigma) * grid, 2)
    gauss = np.exp(-0.5 * vect)
    rv = multivariate_normal([0, 0], [[1, 0], [0, 1]])
    u_matrix = np.array([[np.cos(theta), -np.sin(theta)],
                        [np.sin(theta), np.cos(theta)]])
    skew_mat = [[alpha, 0], [0, alpha]]
    rot_grid = np.dot(np.dot(skew_mat, u_matrix.T), np.transpose(grid, axes=(0, 2, 1)))
    skewd=rv.cdf(np.transpose(rot_grid, axes=(1, 2, 0)))
    return gauss*skewd
...
def bivariate_Skewed(kernel_size, sig_x, sig_y, theta, alpha, grid=None, isotropic=True):
    """Generate a bivariate isotropic or anisotropic Gaussian kernel.
    ...
    if isotropic:
        ...
        sigma_matrix = sigma_matrix2(sig_x, sig_y, theta)
    kernel = skew2(sigma_matrix, grid, alpha, theta)
...
def random_bivariate_skewed(kernel_size,
                            sigma_x_range,
                            sigma_y_range,
                            rotation_range,
                            alpha_range,
    ...
    kernel = bivariate_Skewed(
        kernel_size, sigma_x, sigma_y, rotation, alpha, isotropic=isotropic)
...
def random_mixed_kernels(
    ...
    Args:
        kernel_list (tuple): a list name of kernel types,
            support ['iso', 'aniso', 'generalized', 'plateau_iso',
            'plateau_aniso']
    ...
    elif kernel_type == 'skewed':
        kernel = random_bivariate_skewed(
            kernel_size,
            sigma_x_range,
            sigma_y_range,
            rotation_range,
...
def random_mixed_kernels_helper(kernel_list,
    kernel_prob,
    kernel_size=21,
    sigma_x_range=(0.6, 5),
    sigma_y_range=(0.6, 5),
