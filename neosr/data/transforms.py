...
def single_random_crop(img_gts, gt_patch_size):

    'randomly crops single image'
...
def paired_random_crop_ow(img_gts, img_lqs, gt_patch_size, scale, gt_path=None):
    """Paired random crop (One way). Support Numpy array and Tensor inputs.
    ...
    # randomly choose top and left coordinates for lq patch
    top = random.randint(0, h_lq - lq_patch_size*scale)
    left = random.randint(0, w_lq - lq_patch_size)

    # crop lq patch
    if input_type == "Tensor":
        img_lqs = [
            v[:, :, top : top + lq_patch_size * scale, left : left + lq_patch_size]
            for v in img_lqs
        ]
    else:
        img_lqs = [
    ...
    # crop corresponding gt patch
    top_gt, left_gt = int(top), int(left * scale)
    if input_type == "Tensor":
        img_gts = [
            v[:, :, top_gt : top_gt + gt_patch_size, left_gt : left_gt + gt_patch_size]
            for v in img_gts
